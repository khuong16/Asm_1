...
public class Main {
    ...
    // Khởi tạo đối tượng hashmap để lưu dữ liệu theo kiểu key - value:
    // Ví dụ: mã đơn vị hà nội là Hà Nội với mã số 001, hà giang với mã số 002.
    // Em có thể tham khảo thêm về hashmap ở đây:
    // https://viettuts.vn/java-collection/hashmap-trong-java
    public static HashMap<String, String> danhSachDonViMap = new HashMap<>();

    // Tạo mảng 2 chiều để lưu dữ liệu về giới tính
    ...
    public static void main(String[] args) {
        ...
        // chỗ này anh thêm dữ liệu vào hashmap, em có thể tách ra hàm riêng cũng đc nhé, tránh để trong hàm main
        ...
        System.out.println("+-------+-----------------+----------+");
        System.out.println("|  NGÂN HÀNG SỐ  | Binh @v1.0.0   |");
        System.out.println("+-------+-----------------+----------+");
    ...
    private static void chucNang() {
        ...
        do {
            try {
                ...
                if (input == 1) {  // Kiểm tra xem biến input nhập vào là giá trị bao nhiêu, nếu là 1 sẽ thực thi gì ? nếu là 0 sẽ thực thi gì ?
                    maXacThuc();   // Nếu input là 1 sẽ thực thi hàm maXacThuc
                    break; // Thoát khỏi vòng lặp do-while
                }
                if (input == 0) {   // Kiểm tra xem giá trị input là 0 sẽ thực hiện chức năng gì
    ...
    // thực hiện nhập vào mã xác thực
    // mã xác thực cũng có 2 chế độ là: easy và hard.
    // đối với chế độ easy: chỉ random ra là số, toàn là số.
    // đối với chế độ hard: random ra cả số và chữ cái.
    ...
    private static void checkCCCD() {
        ...
        do {
            // nhập vào CCCD
            inputCCCD = sc.nextLine().trim();

            // kiểm tra xem CCCD có trống hay không
            if (!inputCCCD.isEmpty()) {
                // kiểm tra CCCD theo 3 điều kiện:
                // 1. giá trị cccd không là no.
                // 2. độ dài cccd phải là 12.
                // 3. kiểm tra 3 kí ự đầu tiên của cccd
                // 4. cccd bắt buộc phải là số từ 0-9 ( mathces \\d+ )
                if (!inputCCCD.equals("no") && (inputCCCD.length() != 12 || !isFirstThreeDigitsValid(
                        inputCCCD) || !inputCCCD.matches("\\d+"))) {
                    System.out.println(
            ...
            // !isFirstThreeDigitsValid(
            //                        inputCCCD) ||
        } while (inputCCCD.length() != 12 || !isFirstThreeDigitsValid(inputCCCD) || !inputCCCD.matches("\\d+"));
        checkNoiSinhNamSinhGioiTinh(inputCCCD);
    }

    // kiểm tra 3 kí tự đầu tiên của CCCD
